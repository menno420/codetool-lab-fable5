...
5. **Branch deletion:** 403 per a sibling lane's shared memory (not independently re-tested here — treat as blocked). *(Note: this is a **deletion** wall only. It is NOT a merge wall — merging / self-merge on green is verified working, see "What verifiably WORKS" below.)*
...
8. **No scheduler for the coordinator:** `Error: No such tool available: mcp__claude-code-remote__send_later` — no timed self-wakes; coordination is event-driven only. Do not promise timed follow-ups. *(2026-07-10 rider, reaffirmed 2026-07-18: this is NOT a standing wall — `create_trigger` / `send_later` arm routines agent-side (proven fleet-wide; multiple lanes run live self-armed routines; see fleet-manager `docs/capabilities.md`). The "No such tool available" error above was only this one coordinator seat's verbatim outcome; a seat that exposes the scheduling tools should just use them. Arming a routine is normal agent work, not owner-gated.)*
